...
// SAVE DATA
    if (isset($_POST)) {
        if (!empty($_POST['codigoProdutoEditar']) && !empty($_POST['nomeProdutoEditar']) && !empty($_POST['descricaoEditar']) && !empty($_POST['quantidadeEditar']) && !empty($_POST['precoVendaEditar']) && !empty($_POST['precoCompraEditar']) && !empty($_POST['categoriaEditar'])) {
...
//echo $insert;
            if ($insert) {
                mkdir('C:/xampp/htdocs/threadTional/View/imagensProduto/' . $insert, 0777, true); // Cria uma nova pasta dentro do diretório atual
